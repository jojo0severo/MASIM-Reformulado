...
def connect_actors():
    ...
    carrying_socket.emit('connect_registered_asset', data=json.dumps({'token': token}))
...
def carry_action_results(msg):
    ...
    carrying_socket.emit('disconnect_registered_asset', data=json.dumps({'token': token}), callback=quit_program)
...
def carried_action_results(msg):
    ...
    carried_socket.emit('disconnect_registered_agent', data=json.dumps({'token': carried_token}), callback=quit_program)
