...
def simulation_started(msg):
    ...
    requests.post(URL, json=json.dumps({'token': token, 'action': 'move', 'parameters': victim_loc}))
...
def action_result(msg):
    ...
    if not msg['social_asset']['route']:
        if msg['social_asset']['last_action'] == 'rescueVictim':
            ...
            requests.post(URL, json=json.dumps({'token': token, 'action': 'rescueVictim', 'parameters': []}))
